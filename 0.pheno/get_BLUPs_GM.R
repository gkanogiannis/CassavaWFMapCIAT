...
experiments.names <- list("GM_cnt2016_Exp1_Ane","GM_cnt2017_Exp2_Ane","GM_cnt2018_Exp3_Ane",
                          "GM_are2017_Exp2_Ane","GM_are2018_Exp3_Ane")
...
write.table(pheno.all,file="pheno_GM.all.tsv",sep="\t",quote=F,row.names=F)

BLUP.all <- data.frame(clone=character(),stringsAsFactors=F)
pdf(file = "BLUP_GM.plots.pdf",paper="usr",width=11, height=8.5)
for(experiment in experiments.names){
  #experiment <- "GM_cnt2016_Exp1_Ane"
  ...
  if(length(levels(pheno.experiment$rep))<2){
    formula0 <-" ~ 1 + 
    (1|clone)+(1|plant)+(1|leaf)+
    (1|clone:plant)+(1|clone:leaf)+
    (1|plant:leaf)"
  }
  model <- lmer(as.formula(paste0(experiment,formula0)),data=pheno.experiment)
...
dev.off()
write.table(BLUP.all,file=paste0("BLUPs_GM.csv"),sep="\t",quote=F,row.names=F,col.names=T)
